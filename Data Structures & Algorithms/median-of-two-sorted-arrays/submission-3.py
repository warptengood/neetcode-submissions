    ...
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        while l <= r:
            ...
            if len(nums1) < first_half:
                ...
                max_element = max(first_max_element, second_max_element)
                bonus = min(
                    nums1[first_half] if first_half < len(nums1) else int(2e9), 
                    nums2[second_half] if second_half < len(nums2) else int(2e9)
                ) if n % 2 == 0 else max_element

                if first_max_element == second_max_element:
                    return (first_max_element + bonus) / 2

                n_less_elements = self.bin_less(max_element, nums2 if first_max_element > second_max_element else nums1)
                if first_max_element > second_max_element and n_less_elements > second_half:
                    r = first_half - 1
                elif second_max_element < first_max_element and n_less_elements > first_half:
                    l = first_half + 1
                else:
                    return (max_element + bonus) / 2
